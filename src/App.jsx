...
function App() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "appointments" }, () => loadAppointments(false))
      .subscribe((status) => {
  ...
  async function saveAppointment(e) {
    ...
    if (!form.appointment_date || !form.patient_name || !form.phone) {
      showToast("أدخل التاريخ واسم العميل ورقم الجوال");
      return;
    }
    const payload = {
    ...
    showToast(form.id ? "تم تعديل البيانات" : "تم إضافة البيانات");
  ...
  async function importExcel(event) {
    ...
    reader.onload = async (e) => {
      ...
      const imported = rows.map((r) => ({
        ...
        status: String(r["الحالة"] || r.status || "معلق"),
  ...
  return (
    <main className="app">
      <section className="hero">
        <div className="heroText">
          ...
          <div>
            ...
            <p>نظام لايف لمتابعة العملاء، إرسال الروابط، الاستشارات الطبية، والحجوزات.</p>
        ...
        <div className="heroActions">
          <button className="btn btnLight" onClick={openAddModal}><Plus size={18} /> إضافة سجل</button>
          <button className="btn btnPrimary" onClick={exportExcel}><Download size={18} /> تصدير Excel</button>
          <label className="btn btnGhost fileBtn">استيراد Excel<input type="file" accept=".xlsx,.xls" onChange={importExcel} /></label>
        </div>
      </section>
      ...
      <section className="panel filters">
        <div className="field searchField"><label>بحث</label><div className="inputIcon"><Search size={18} /><input value={filters.q} onChange={(e) => setFilters({ ...filters, q: e.target.value })} placeholder="الاسم، الجوال، الملاحظات..." /></div></div>
        <div className="field"><label>التاريخ</label><input type="date" value={filters.date} onChange={(e) => setFilters({ ...filters, date: e.target.value })} /></div>
        <div className="field"><label>الحالة</label><select value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })}><option value="">كل الحالات</option>{statuses.map((s) => <option key={s}>{s}</option>)}</select></div>
        <button className="btn btnGhost" onClick={() => loadAppointments()}><RefreshCw size={16} /> تحديث</button>
      </section>

      <section className="panel">
        {loading ? <div className="empty">جاري تحميل البيانات...</div> : (
          <div className="tableWrap"><table><thead><tr><th>التاريخ</th><th>اسم العميل</th><th>رقم الجوال</th><th>الحالة</th><th>ملاحظات</th><th>تاريخ الإضافة</th><th>إجراءات</th></tr></thead><tbody>
            {filtered.length === 0 ? <tr><td colSpan="7" className="empty">لا توجد بيانات مطابقة</td></tr> : filtered.map((item) => {
              const phone = normalizePhone(item.phone);
              return <tr key={item.id}><td>{item.appointment_date}</td><td><strong>{item.patient_name}</strong></td><td><div>{item.phone}</div>{phone.startsWith("966") && <a className="whatsapp" href={`https://wa.me/${phone}`} target="_blank" rel="noreferrer"><Phone size={14} /> واتساب</a>}</td><td><span className="badge">{item.status}</span></td><td className="notes">{item.notes}</td><td>{item.created_at ? new Date(item.created_at).toLocaleString("ar-SA") : ""}</td><td><div className="rowActions"><button className="miniBtn" onClick={() => openEditModal(item)}><Edit size={15} /></button><button className="miniBtn danger" onClick={() => deleteAppointment(item.id)}><Trash2 size={15} /></button></div></td></tr>;
            })}
          </tbody></table></div>
        )}
      </section>

      {isModalOpen && <div className="modalOverlay" onMouseDown={(e) => e.target.className === "modalOverlay" && setIsModalOpen(false)}><div className="modal"><div className="modalHead"><h2>{form.id ? "تعديل سجل" : "إضافة سجل"}</h2><button className="closeBtn" onClick={() => setIsModalOpen(false)}><X size={20} /></button></div><form onSubmit={saveAppointment} className="form">
        ...
        <Field label="ملاحظات" wide><textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows="4" placeholder="أي ملاحظات مهمة" /></Field>
        <div className="modalFooter"><button className="btn btnPrimary" type="submit">{form.id ? "حفظ التعديل" : "حفظ السجل"}</button><button className="btn btnGhost" type="button" onClick={() => setForm(emptyForm())}>تفريغ</button></div>
      </form></div></div>}
